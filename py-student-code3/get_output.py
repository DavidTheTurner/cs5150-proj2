...
x[4] = np.array(
    [5.4297, 5.2275, 2.9865, 1.4532, 2.1778, 3.2898, 6.8113, 9.0691, 7.2999, 7.2157, 9.2713, 7.4853, 6.4575]
)
y[4] = np.array(
    [4.6494, 1.9055, 1.7429, 3.8974, 8.0030, 6.7429, 9.1209, 7.2917, 6.4380, 2.8404, 2.7795, 0.9502, 1.3974]
)
# More test points
...
for i in range(len(x)):
    ...
    write_table(
        fname,
        [f"Bx_{j+1}" if j < Bx.shape[1] else f"By_{j+1-Bx.shape[1]}" for j in range(Bx.shape[1] + By.shape[1])],
        [Bx[:, j] if j < Bx.shape[1] else By[:, j-Bx.shape[1]] for j in range(Bx.shape[1] + By.shape[1])]
    )
